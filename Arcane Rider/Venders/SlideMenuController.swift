...
public struct SlideMenuOptions {
    ...
    public static var animationDurationBack: CGFloat = 0.1
	...
	public static var opacityViewBackgroundColor: UIColor = UIColor(red: 235.0/255.0, green: 235.0/255.0, blue: 235.0/255.0, alpha: 1.0)
...
open class SlideMenuController: UIViewController, UIGestureRecognizerDelegate {
    ...
    // extra func back call //check
    
    open override func openLeftBackExtra() {
        guard let _ = leftViewController else { // If leftViewController is nil, then return
            return
        }
    ...
    open func openLeftWithVelocityBack(_ velocity: CGFloat) {
        let xOrigin: CGFloat = leftContainerView.frame.origin.x
        let finalXOrigin: CGFloat = 0.0
        ...
        var duration: TimeInterval = Double(SlideMenuOptions.animationDurationBack)
        if velocity != 0.0 {
            duration = Double(fabs(xOrigin - finalXOrigin) / velocity)
            duration = Double(fmax(0.1, fmin(1.0, duration)))
        }
...
extension UIViewController {
    ...
    public func openLeftBackExtra() {
        
        slideMenuController()?.openLeftBackExtra()
    }
